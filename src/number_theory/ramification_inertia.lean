...
where `p` is an ideal of `R` such that `R / p` is nontrivial and `S` is the integral closure of
`R` in `L`.
...
lemma finrank_quotient_map [is_domain R] [is_domain S] [is_dedekind_domain R]
  ...
  [is_integral_closure S R L] (hRL : function.injective (algebra_map R L))
...
here `S` is a finite `R`-module (and thus `Frac(S) : Frac(R)` is a finite extension), `p` is maximal
-/
